...
import type { UserCourse, CreateCourseRequest, UpdateProgressRequest, SubmitAnswersRequest } from '@/types/user-course';
...
interface UseUserCoursesReturn {
  ...
  updateProgress: (courseId: string, progressData: UpdateProgressRequest) => Promise<void>;
  submitAnswers: (courseId: string, submissionData: SubmitAnswersRequest) => Promise<any>;
...
export function useUserCourses(): UseUserCoursesReturn {
  ...
  // 获取特定课程详情
  const getCurrentCourse = useCallback(async (courseId: string): Promise<UserCourse | null> => {
    try {
  ...
  // 创建新课程
  const createCourse = useCallback(async (courseData: CreateCourseRequest): Promise<UserCourse> => {
    try {
      setLoading(true);
      setError(null);
  ...
  const submitAnswers = useCallback(async (
    ...
    try {
      const response = await fetch(`/api/user-courses/${courseId}/submit-answers`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },

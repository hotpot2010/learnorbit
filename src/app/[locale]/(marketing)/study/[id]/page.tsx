...
import type { LearningPlan, LearningStep, TaskGenerateRequest, TaskGenerateResponse, TaskContent, QuizQuestion, CodingTask } from '@/types/learning-plan';
  ...
  useEffect(() => {
    const resolveParams = async () => {
      ...
      if (resolvedParams.id === 'custom') {
        ...
        // 本地无缓存，按 slug 拉取
        try {
          console.log('🔎 按 slug 拉取课程:', slug);

          // 首先尝试创作者课程API（简洁URL，不包含用户ID）
          let resp = await fetch(`/api/creator-courses/${encodeURIComponent(slug)}`);
          let isCreatorCourse = false;

          // 如果创作者课程API失败，回退到普通公共课程API
          if (!resp.ok) {
            console.log('🔄 创作者课程未找到，尝试普通公共课程API');
            resp = await fetch(`/api/public-courses/${encodeURIComponent(slug)}`);
          } else {
